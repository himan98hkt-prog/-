...
| **플레이어 곡목록·연주** | **조심** | 오프라인 꾸러미입니다 (지금 **194 KB** 전부) |
...
| 파일 | 무엇 | 크기 |
|---|---|---|
| `mr/server/static/img/hall.mp4` | 무대 배경 영상 — **격식 있는 대강당** (Kling v3.0 pro) | 1904×1088 · 10초 · 무음 · 2.1 MB |
| `mr/server/static/img/hall.jpg` | 같은 홀의 정지 사진 (영상이 안 뜰 때) | 2400×1350 · 166 KB |
...
무대는 **셋 중 하나만** 씁니다. 처음엔 「아담한 리사이틀홀」로 붙였다가 원장님 요청으로
**「격식 있는 대강당」**으로 바꿨습니다 — 장식 프로시니엄과 안개를 가르는 빛기둥이 더
극적이고, 왼쪽이 어두워 아이 이름이 더 잘 읽힙니다. 아담한 홀로 되돌리시려면 말씀만
주세요. 「현대 미니멀」안은 프롬프트가 안 먹어(내장 로케이션으로 그리는 모델) 피아노가
작고 구도가 밋밋해 권하지 않습니다.
