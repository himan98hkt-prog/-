...
| **플레이어 곡목록·연주** | **조심** | 오프라인 꾸러미입니다 (지금 **161 KB** 전부) |
...
**제 컨테이너에서는 결과 파일을 못 받습니다.** 이 작업 환경의 바깥 통신은 **허용
목록 방식**이라 GitHub·npm·PyPI 말고는 거의 다 막혀 있습니다 — 힉스필드 CDN 도,
결과가 놓이는 CloudFront 도, 심지어 `www.google.com` 도 CONNECT 에 403 입니다.
힉스필드 쪽 문제가 아니고 일시적인 것도 아닙니다. 정책 거부는 우회하지 않는 것이
원칙이라 받아서 커밋하지 못했습니다.
...
| 길 | 어떻게 | 언제 |
|---|---|---|
| **채팅에 그냥 붙이기** (제일 쉽습니다) | 위젯에서 내려받아 **대화창에 첨부**하시면 됩니다. 제가 집어서 줄이고, 자리에 넣고, 검사 돌리고, 커밋·푸시까지 합니다 | 저장소를 컴퓨터에 안 받아 두셨어도 됩니다 |
| 저장소에 직접 커밋 | 아래 `curl` 로 받아 폴더에 넣고 `git commit` · `git push` | 컴퓨터에 저장소를 받아 두셨다면 |
| 웹 어딘가에 올려 주소 주기 | GitHub 에 올리시면 제가 받을 수 있습니다 (GitHub 은 허용 목록에 있습니다) | 파일이 많을 때 |

**어느 쪽이든 제가 할 일은 같습니다** — 크기 줄이기, 자리 잡기, `player_check`
11항목 돌리기, 스크린샷·설명서 다시 만들기, 커밋·푸시.
