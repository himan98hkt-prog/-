...
- **buyable.can_buy 가 false 인 종목에는 BUY 를 내지 않는다.** 한도·규정상 지금
  살 수 없는 종목이라, BUY 를 줘도 주문이 되돌려 보내질 뿐 매수 제안 한 장만
  낭비된다. buyable.reason 에 이유가 적혀 있다. 그런 종목은 보유 판단(HOLD/SELL)
  만 하고, 매수 자리는 실제로 살 수 있는 종목에 준다.
- buyable.room_krw 는 그 종목에 지금 더 넣을 수 있는 금액이다. weight_pct 를
  정할 때 참고한다 — 여유보다 큰 비중을 제안해도 그만큼은 집행되지 않는다.
