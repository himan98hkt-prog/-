...
# 후보 전부를 한 번에 볼 때는 답이 종목 수만큼 길어진다. 추론 모델은 사고 토큰도
# 이 한도를 함께 쓰므로 넉넉히 준다 — 모자라면 JSON 이 잘려 파싱 실패가 된다.
    ...
    def __init__(self, env: EnvConfig, ai: AiConfig, client=None) -> None:
        ...
        if client is not None:
            ...
            self.client = OpenAI(api_key=env.openai_api_key,
                                 timeout=ai.client_timeout_sec, max_retries=0)

    def _request_kwargs(self, system_prompt: str, user_prompt: str, schema: dict) -> dict:
        kwargs = {
            "model": self.model,
            "messages": [
            ...
            "max_completion_tokens": (BATCH_MAX_OUTPUT_TOKENS
                                      if schema is BATCH_RESPONSE_JSON_SCHEMA
