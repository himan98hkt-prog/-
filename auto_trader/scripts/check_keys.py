...
class Result:
    ...
    def line(self) -> str:
        return f"{self.status} {self.name:<22} {self.detail}"
...
def main() -> int:
    ...
    results.extend(check_telegram(env, send_test=args.telegram_test))
    ...
    required_skipped = [r for r in results if r.status == SKIP and "선택" not in r.name]

    print("-" * 70)
    if failed:
        print(f"❌ {len(failed)}개 항목이 실패했습니다. 위 메시지를 확인해 .env 를 고친 뒤 다시 실행하세요.")
        return 1
    if required_skipped:
        print(f"⏭️  {len(required_skipped)}개 항목이 미입력 상태입니다. 전부 채워야 프로그램이 기동합니다.")
        return 1
