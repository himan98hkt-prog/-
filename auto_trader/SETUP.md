...
**프로젝트에 결제 계정을 연결하지 마세요.** 결제가 연결되지 않은 프로젝트의
키는 무료 등급으로 동작합니다. Flash 모델 기준 **분당 15회 / 하루 1,500회** 인데,
이 프로그램은 **하루 13회**(30분 사이클 기준) 씁니다 — 100배 여유입니다.

- `GEMINI_MODEL` 은 기본값 `gemini-3.5-flash` 그대로 두세요. Pro 모델은 2026년
  4월부터 무료 등급에서 빠졌습니다.
- 한도에 걸리면 해당 종목은 자동으로 HOLD 처리되고 프로그램은 멈추지 않습니다.
