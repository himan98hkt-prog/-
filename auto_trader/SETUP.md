...
### 무료 등급으로 쓰려면 — 사이클을 60분으로 늘려야 합니다

구글은 무료 등급 한도를 자주 바꿉니다. **2026년 9월 현재 `gemini-3.5-flash` 는
하루 20회**입니다(예전에는 훨씬 넉넉했습니다). 실제 한도는 계정마다 다를 수
있으니, 아래 링크에서 본인 키의 한도를 확인하세요.

https://ai.dev/rate-limit

이 프로그램이 Gemini 를 부르는 횟수는 **사이클 수와 같습니다**(후보 전부를
한 번에 묻는 상대평가라 사이클당 1회).

| 사이클 주기 | 하루 호출 | 하루 20회 한도에서 |
|---|---|---|
| 30분 (기본) | 13회 | ⚠️ 재시도 한 번이면 바로 초과 |
| **60분** | **7회** | ✅ 여유 있음 |

무료 등급을 쓰시려면 `config/settings.yaml` 의
`schedule.cycle_interval_min` 을 **60** 으로 바꾸세요. 3~10거래일 스윙 판단이라
30분마다 되물어도 그림이 크게 바뀌지 않고, 손절 감시는 3분마다 따로 돕니다.

**받는 법**: 결제 계정을 연결하지 않은 프로젝트에서 키를 만드세요. 결제가
연결되면 자동으로 유료 등급이 됩니다(Flash 기준 월 2~3달러 수준).

- `GEMINI_MODEL` 은 기본값 `gemini-3.5-flash` 그대로 두세요. Pro 모델은 2026년
  4월부터 무료 등급에서 빠졌습니다.
- 한도에 걸리면 **그 엔진은 오늘 건너뜁니다**(계속 부르면 한도만 더 축냅니다).
  Gemini 가 빠진 사이클은 매수가 나가지 않습니다 — 매수는 만장일치라서요.
  손절·익절은 AI 와 무관하게 계속 동작합니다.
- 무료 등급은 보내는 내용이 구글 서비스 개선에 쓰일 수 있습니다. 이 프로그램이
  보내는 것은 공개된 시세와 기술 지표뿐이고 계좌 정보나 키는 보내지 않습니다.

이미 유료 키를 쓰고 계시다면, 결제를 연결하지 않은 **새 프로젝트**에서 키를
새로 발급받아 `GEMINI_API_KEY` 만 갈아 끼우면 됩니다.
...
대시보드는 **127.0.0.1(이 PC)에만** 열려 있습니다. 키를 입력하고 매매를 멈출 수
있는 화면이라 일부러 그렇게 두었습니다. 폰에서 보는 길은 두 가지입니다.

### 방법 1 — 텔레그램 (설치할 것 없음)

이미 알림을 받고 계시다면 그대로 됩니다. 봇에게 명령을 보내세요.

| 명령 | 보여 주는 것 |
|---|---|
| `/상태` | 모드·당일 손익·평가자산·보유·오늘 주문·AI 비용·다음 사이클·휴장일 |
| `/보유` | 평단→현재, 손절까지·익절까지 남은 거리, 트레일링 기준가 |
| `/판단` | 최근 AI 판단과 그 결과 (왜 샀는지·왜 안 샀는지) |
| `/오늘` | 당일 요약과 리스크 규칙이 막은 것 |
| `/정지` `/재개` | 새 사이클 멈추기 / 되돌리기 |

**폰으로 주문을 낼 수 있는 명령은 없습니다.** 보기와 멈추기뿐이고,
등록된 채팅(`TELEGRAM_CHAT_ID`)에서 온 메시지만 받습니다.

### 방법 2 — Tailscale (대시보드 화면 전체)

내 기기들끼리만 연결되는 암호화 통로를 만듭니다. **대시보드는 127.0.0.1에
묶인 그대로** 두고, Tailscale이 그걸 중계합니다. 인터넷에 공개되지 않습니다.

1. **PC**: https://tailscale.com/download/windows 에서 받아 설치 → 로그인
   (구글 계정 등으로 가입. 개인 사용은 무료)
2. **폰**: 앱스토어/플레이스토어에서 `Tailscale` 설치 → **같은 계정**으로 로그인
3. **PC**: 프로그램 폴더의 **`mobile.bat`** 을 더블클릭
   - 관리자 권한을 물으면 예를 누르세요 (Tailscale 공유에 필요합니다)
   - 처음 한 번은 "HTTPS를 켜시겠습니까" 링크가 뜹니다. 눌러서 켜고 다시 실행
4. 창에 나온 `https://...ts.net` 주소를 **폰에서** 엽니다

주소는 폰 홈 화면에 추가해 두면 앱처럼 씁니다. PC를 재부팅해도 유지됩니다.

끄려면 **`mobile.bat off`** (명령 프롬프트에서) 또는 Tailscale 앱에서 로그아웃.

> **`tailscale funnel` 은 쓰지 마세요.** 이름이 비슷하지만 그건 **인터넷 전체에**
> 공개하는 기능입니다. 설정 화면에 API 키 입력란과 매매 정지 버튼이 있습니다.
>
> 같은 이유로 대시보드를 `0.0.0.0` 으로 여는 것도 권하지 않습니다. 집 와이파이에
> 접속한 누구나 설정 화면을 열 수 있게 됩니다.
