...
> **현재 상태: 주문 안전성 강화 베타. 실전 승인 아님.** [SAFETY_UPGRADE.md](SAFETY_UPGRADE.md)의 변경사항·이전 절차·미검증 항목을 먼저 읽으세요. 수익성 백테스트 및 실제 API/모의계좌 검증은 별도입니다.
...
**Windows** — `install.bat` 을 한 번 실행하면 바탕화면에 **자동매매** 아이콘이 생깁니다.
그 다음부터는 아이콘만 더블클릭하세요. (아이콘 없이 `start.bat` 을 직접 눌러도 같습니다.)
...
현황 화면 맨 위 띠에 지금 어느 쪽인지 항상 보이고, 그 자리에서 바로 바꿉니다.
실전은 빨간 띠, 모의투자는 파란 띠입니다.

실행 중에는 환경 전환을 거부합니다. 종료 후 전환하면 DRY_RUN=true 및 실전 잠금으로 돌아갑니다.
DB는 하나의 계좌/환경에 고정되므로 모의/실전은 별도 설치를 사용하세요.

> **계좌번호도 같이 바꿔야 합니다.** 모의계좌와 실계좌는 번호가 다릅니다.

### 폰에서 보고 멈추기 (텔레그램)

매매는 PC 가 돌리고, 폰은 리모컨입니다. 자동매매가 떠 있으면 텔레그램 봇에
명령을 보낼 수 있습니다.

| 명령 | 하는 일 |
|---|---|
| `/상태` | 가동 여부·모드·당일 손익·다음 사이클 |
| `/보유` | 보유 종목과 평가손익 |
| `/오늘` | 당일 매매 요약 |
| `/정지` | 긴급 정지 (새 사이클 중단) |
| `/재개` | 정지 해제 |

- **등록된 `TELEGRAM_CHAT_ID` 에서 온 명령만** 받습니다. 남이 봇 이름을 알아내
  말을 걸어도 무시하고 답장조차 하지 않습니다.
- **폰으로 주문을 새로 낼 수는 없습니다** — 보기와 멈추기만 됩니다.
- 계좌번호·API 키는 어떤 명령으로도 나오지 않습니다.
- 정지 중에는 **손절·익절을 포함한 모든 새 주문을 차단**합니다. 이미 접수된 주문은 자동 취소하지 않습니다.

### PC 재시작 후에도 이어가기

현황 화면의 **🔁 PC 시작 시 자동 실행** 버튼을 누르면 윈도우 시작 프로그램에 등록됩니다.
윈도우 업데이트로 재부팅돼도 대시보드와 자동매매가 스스로 다시 뜹니다.

- 부팅 후 **30초 기다렸다가** 시작합니다(네트워크가 잡히기 전에 증권사 API 를 부르면 실패).
- 설정이 덜 됐거나 이미 돌고 있으면 아무것도 하지 않습니다.
- 관리자 권한도, 비밀번호 저장도 필요 없습니다.

> **한계**: 시작 프로그램은 **윈도우에 로그인해야** 실행됩니다. 재부팅 후 잠금화면에
> 머물러 있으면 그동안은 매매도 멈춰 있습니다. 자동 로그인을 켜 두거나 장 시작 전에
> 한 번 로그인해 두세요.

### 업데이트 — 버튼 하나

현황 화면의 **⬆️ 업데이트** 버튼을 누르면 최신 코드를 받아 제자리에 적용합니다.
**다시 내려받거나 압축을 풀 필요도, 키를 다시 입력할 필요도 없습니다.**

| 건드리지 않는 것 | 이유 |
|---|---|
| `.env` | 입력한 API 키. 몇 번을 업데이트해도 그대로입니다 |
| `data/` | 매매 기록·DB·당일 손익 |
| `logs/` | 지난 로그 |
| `config/settings.yaml` | 직접 고친 매매 파라미터. 새 파일은 `.yaml.new` 로만 둡니다 |

봇이 돌고 있으면 알아서 멈췄다가 새 코드로 다시 띄웁니다. **대시보드 자신도 몇 초 뒤
자동으로 다시 뜹니다** — 파이썬은 모듈을 한 번만 읽으므로, 프로세스를 갈지 않으면
화면이 예전 그대로입니다(새 입력칸이 안 생깁니다). 업데이트에 실패하면 기존 설치를
이전 코드로 복구를 시도합니다. 실패 시 자동매매는 재시작하지 않으며 수동 확인이 필요합니다.
전원 차단까지 원자적으로 복구되는 업데이트는 아니므로 보유·미체결이 없는 장외 시간에만 사용하세요.

> 새 패키지가 필요한 업데이트만 예외입니다. 이때는 창을 닫고 `start.bat` 을 다시 실행하세요.

터미널에서도 됩니다:

```bash
python scripts/update.py --check   # 새 버전이 있는지만 확인
python scripts/update.py           # 적용
```
Windows 는 `update.bat` 더블클릭.
...
├── data_pipeline/          유니버스·시세/지표 수집                        (Step 3)
...
- 호가 수집 실패는 기본값으로 넘어가고, 현재가·일봉 실패만 예외로 올립니다(판단 근거가 없으므로).
...
| 3 | 데이터 파이프라인 (유니버스·지표) | ✅ 코드·테스트 완료 / 실연동 검증 대기 |
