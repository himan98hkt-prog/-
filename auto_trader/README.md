...
### 후보를 함께 본다 — 상대평가 (`ai.compare_candidates`)

종목을 하나씩 던지고 "이거 살 만한가"를 물으면 **절대평가**입니다. 절대평가의 기준선은 높고,
대부분의 날은 어느 종목도 그 선을 넘지 못해 답이 관망으로 쏠립니다. 실제 자금은
"이 종목이 좋은가"가 아니라 **"이 중 어디에 넣을 것인가"**로 배분되므로, 후보를 한 번에
보여주고 서로 비교해 순위를 매기게 합니다.

| 설정 | 기본값 | 뜻 |
|---|---|---|
| `ai.compare_candidates` | `true` | 후보 전부를 한 번에 보여준다. `false` 면 예전처럼 종목별로 따로 묻는다 |
| `ai.max_buy_picks` | `3` | 한 번에 매수 제안을 줄 수 있는 상한 |
| `ai.batch_timeout_sec` | `180` | 후보 전부를 한 번에 볼 때의 대기 한도 |

- 프롬프트에 **안 사는 비용**을 명시합니다 — "현금의 수익률은 0%다". 반대쪽(확신 없는 매수의
  수수료·세금)도 함께 적어, 한쪽으로만 안전하게 기울지 않게 합니다.
- **억지로 채우지 않습니다.** 살 자리가 없으면 전부 HOLD 로 답해도 된다고 명시합니다.
- 응답에서 **후보가 하나라도 빠지면 오류**로 보고 다시 묻습니다. 조용히 HOLD 로 채우면
  "관망했다"와 "빠뜨렸다"가 섞여 합의 통계도 엔진 적중률도 믿을 수 없게 됩니다.
- 한 엔진의 상대평가가 끝내 실패하면 **그 엔진만** 종목별 호출로 되돌립니다. 응답 하나가
  깨졌다고 그 사이클의 매수를 통째로 막지 않기 위해서입니다.
- 손절·트레일링 대상 종목은 애초에 묻지 않습니다(되묻는 사이에 더 밀립니다).
- 시스템 프롬프트가 종목 수만큼 반복되지 않으므로 **입력 토큰은 오히려 줄어듭니다.**

합의 규칙 자체는 아래 그대로입니다 — 상대평가는 *질문하는 방식*을 바꾼 것이지
*매수 문턱*을 낮춘 것이 아닙니다.
...
| 사이클 | `first_cycle`(09:05)부터 `cycle_interval_min`(30분) 간격, **15:00까지** | 후보 전체 수집 → 상대평가 → 합의 → 리스크 → 주문 |
