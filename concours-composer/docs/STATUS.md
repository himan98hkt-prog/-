...
현재 마일스톤: **M0·M2·M3a·M3b·M5 코어 완료 · 세션 엔진으로 5곡 실작곡 완료**
테스트 146건 통과 · 골든 20건 회귀 통과 · ruff/mypy 클린 · 다음은 M1(악보 렌더)·M4(에디터)
...
1. **API 를 통한 대량 실측은 아직이다.** 세션 엔진으로 5곡을 실제로 작곡해 파이프라인
   전체가 사람(또는 모델)의 실제 출력으로 돌아간다는 것은 확인했다. 다만 20곡 골든을
...
3. **저장소는 아직 프로세스 메모리다.** `server/app/api/deps.py` 의 `Store` 와
   `app/ingest/corpus.py` 의 `Corpus` 두 곳만 바꾸면 PostgreSQL+pgvector 로 간다.
   서버를 재시작하면 올린 코퍼스와 만든 곡이 사라진다.
