...
현재 마일스톤: **M0·M1(렌더·재생)·M2·M3a·M3b·M5 코어 완료 · 세션 엔진으로 골든 20곡 작곡 완료**
테스트 166건 통과 · 골든 20건 회귀 통과 · ruff/mypy 클린 · 다음은 M4(마우스 에디터)
...
## 작곡 품질 개선 (2026-09-01 3차 — 곡 하나만 보면 안 보이던 것)
- [x] **곡 사이 다양성 검사(Plan 규칙)** — 형식 지문(종지 공식·프레이즈 길이열·모티브 처리·
      클라이맥스 위치·형식 라벨)으로 이미 만든 곡과 대조해 0.60 이상 겹치면 하드 실패.
      초판 다섯 곡은 g01–g02 가 0.75 였고 g01·g02·g03 의 마지막 여섯 마디 화성이 동일했다.
      g02·g03 의 종지를 각각 확인 종지·변격(아멘) 종지로 바꿔 최대 유사도 0.45 로 내렸다.
- [x] **생성곡 코퍼스 자동 등록** — 저장되는 곡이 다음 곡의 표절 인덱스에 들어간다
      (`generated=True` 라 스타일 검색에서는 빠진다 — 자기 출력을 참고하면 곡이 서로 닮는다).
      세션 골든도 앞서 만든 곡들을 코퍼스로 삼는다.
- [x] **루브릭 겨냥 수정 라운드** — 종합 점수가 문턱을 넘어도 개별 루브릭이 7.0 아래면
      그 항목을 겨냥해 한 라운드 실행. 이 패스가 없어서 비평가의 수정 지시 15건이
      **한 건도 실행되지 않았다.** 지금은 다섯 곡 전부 이 라운드를 돌고 채택된다.
- [x] **난이도 되먹임** — 목표에서 허용 오차를 벗어나면 특징별 손잡이("simultaneity 0.22 —
      강박에 3도·6도를 겹쳐라")를 지시에 붙인다. 채택은 점수와 난이도를 합친 목표로 판단.
- [x] **모티브 잠금 구간 보호** — 모티브가 처음 제시되는 마디를 겨냥한 지시는 실행하지 않고
      원장에게 넘긴다. 머리를 바꾸면 되풀이하는 모든 자리가 함께 바뀌어야 하기 때문이다.
- [x] **`find_plagiarism` 길이 버그** — 겹친 마디 수가 항상 `n//2` 로 나왔다(시작 마디부터
      곡 끝까지를 셌다). 48마디 곡 자기 대조에서 195건이 전부 "8마디" 였다. 이제 3~7마디로 나온다.
- [x] **골든에 모의 심사·연주법 해설 연결** — 두 품질 층이 실제 산출물로 한 번도 검증되지
      않았다. 곡마다 `judge.json`·`guide.json` 을 남기고 요약표에 심사 평균을 넣었다.
...
7. **골든의 형식 유사도는 재기만 한다.** 규칙 기반 스텁은 모든 요청에 같은 설계
   뼈대를 내놓으므로 20건의 최대 유사도가 1.00 이고 39쌍이 걸린다. 스텁의 알려진
   한계이며(리포트에 그대로 적는다), 실제 작곡 경로에서는 Plan 규칙이 하드로 막는다.
8. **표현 다양성 지표는 순위를 매기지 못한다.** 제대로 쓴 곡들은 0.85~1.00 에 몰린다.
