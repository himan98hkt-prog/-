...
Stage 3  프레이즈 단위 실현       4마디씩 순차 생성. 앞 8마디를 문맥으로 받는다. 즉시 검증
Stage 4  조립                    JSON → music21 → MusicXML 3.1 (결정적)
Stage 5  채점 + 비평 루프         규칙 지표 8종 + 별도 비평가 호출 → 미달이면 해당 프레이즈만 재생성
...
| **음악성 지표**(§7.4) | 모티브 일관성 · 반복/변화 균형 · 선율 윤곽 · 화성 일치 · 프레이즈 호흡 · 다이내믹 곡선 · 텍스처 대비 · 연주 편의 |
...
화면은 API 를 띄우고 `web/index.html` 을 브라우저로 열면 된다.
