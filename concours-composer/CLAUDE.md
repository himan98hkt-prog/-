...
12. 새 곡의 설계도는 같은 학원이 이미 만든 곡들과 대조한다. 형식·종지 공식·모티브 처리·
    클라이맥스 위치가 함께 겹치면(유사도 0.60 이상) **Plan 단계에서 하드 실패**시킨다.
    곡 하나만 보는 지표는 이것을 절대 잡지 못한다 — 같은 콩쿨에 형제 같은 곡을 내보내는 것이
    개별 곡의 결함보다 크다.
13. 비평가가 낸 수정 지시는 종합 점수가 문턱을 넘어도 버리지 않는다. 개별 루브릭이
    `RUBRIC_FLOOR` 아래면 그 항목을 겨냥해 한 라운드 실행한다. 단 **잠긴 모티브가 처음
    제시되는 마디**와 곡 전체를 가리키는 지적은 실행하지 않고 원장에게 글로 넘긴다.
14. 모델 문자열은 env(COMPOSER_MODEL, WRITER_MODEL)로만 참조. 코드에 하드코딩 금지. 시작 시 docs.claude.com 모델 목록으로 유효성 확인.
