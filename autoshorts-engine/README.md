...
> **업로드 할당량.** `videos.insert` 는 조회용 할당량과 **다른 버킷**에서 집계되며,
> 실제 한도는 Google Cloud 프로젝트마다 다릅니다. 이 도구는 기본값(하루 100건)을
> 느슨한 사전 가드로만 쓰고, **최종 판정은 API 응답**에 맡깁니다. 한도가 다르면
> 코드 수정 없이 `AUTOSHORTS_UPLOAD_DAILY_LIMIT` 로 바꾸세요. 기본값은 아직 공식
> 문서로 재검증되지 않았습니다 — `docs/PHASE0_PRODUCTIZATION_REPORT.md` 의 확인
> 절차를 참고하세요.
