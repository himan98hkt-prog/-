# 첫 영상 만들기 — 30분 가이드 (터미널 버전)

> 💡 **명령어를 치기 싫다면 [START_HERE.md](START_HERE.md) 를 보세요.**
> 바탕화면 아이콘과 브라우저 화면만으로 똑같은 일을 할 수 있습니다.
> 아래 문서는 터미널로 직접 하려는 경우를 위한 것입니다.
...
> 또는 `python main.py ui` 로 작업실을 열고 **[설정] 탭**에 붙여넣어도 된다.
> `.env` 는 자동으로 만들어진다.
