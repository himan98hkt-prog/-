...
"""카탈로그 관리 CLI — 지시서 9장 2단계.
    ...
    python3 catalog_cli.py midi --all
...
from piano_mr import catalog as cat, harmony, orchestration as orch, store  # noqa: E402
...
def cmd_import(st: store.CatalogStore, a) -> int:
    try:
        song = st.import_score(
            ...
            recommended_bpm=a.recommended_bpm or [], note=a.note)
...
def cmd_midi(st: store.CatalogStore, a) -> int:
    ids = _targets(st, a)
    if not ids:
        print('--all 이나 --id 를 주세요.', file=sys.stderr)
        return 1
    total = 0
    for i in ids:
        path = st.build_midi(i)
        size = os.path.getsize(path)
        total += size
        print(f'{i}: {size:,} bytes')
    print(f'\n합계 {total / 1024:.1f} KB — 오디오로 저장했다면 '
          f'{total / 1024 / 1024 * 2300:.0f} MB 였습니다 (지시서 2.4)')
    return 0
...
def build_parser():
    ...
    m = sub.add_parser('midi', help='반주 MIDI 굽기 (카탈로그가 저장하는 유일한 산출물)')
    m.add_argument('--id')
    m.add_argument('--all', action='store_true')
    m.set_defaults(fn=cmd_midi)
