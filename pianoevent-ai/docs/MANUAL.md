...
- **화면에 있는 글씨는 하나도 빠짐없이** 커집니다. 작은 안내 글씨, 표 아래 설명까지
  다 따라옵니다.
- 화면에서 가장 작은 글씨도 **12px 아래로는 나오지 않습니다.**

> 예전에는 안내 글씨 몇 곳이 이 단추를 따라오지 않았습니다. 정작 안 보이는 글씨가
> 그대로 남았던 것입니다. 지금은 `npm run verify:simple` 이 화면마다 재어 보고,
> 12px 아래 글씨가 하나라도 있으면 검사가 실패합니다.
...
로그인하실 것 없이 그 자리에서 바로 받아지는 주소입니다. 아래 **Assets** 에서

- 윈도우 — `RecitalManager-Setup-Windows.exe`
- 맥 — `RecitalManager-Mac.dmg`

**「RecitalManager-Setup-Windows.exe」를 두 번 클릭하고, 다음 · 다음 · 설치.** 그게 전부입니다.

- 바탕화면과 시작 메뉴에 **「연주회 매니저」 아이콘**이 생깁니다. 그 아이콘을 누르면 프로그램 창이 뜹니다.
- **검은 명령창이 뜨지 않습니다.** 주소창도 없습니다. 다른 프로그램과 똑같이 창 하나로 뜹니다.
- **인터넷이 없어도 켜집니다.** 필요한 것이 전부 프로그램 안에 들어 있습니다.
  (학부모께 보내는 초대장 링크만 인터넷이 필요합니다.)
- 지우실 때는 **프로그램 추가/제거**에서 지우시면 됩니다.

> **지워도 학원 명단은 남습니다.** 실수로 지우셨다가 다시 까셔도 아이 명단·사진·행사가
> 그대로 있습니다. 명단은 프로그램이 깔린 자리가 아니라 윈도우가 정해 준 **자료 폴더**에
> 담기기 때문입니다. 어디인지 궁금하시면 메뉴에서 **[연주회 매니저 → 학원 자료 폴더 열기]**.

맥은 **「RecitalManager-Mac.dmg」** 를 열고 응용 프로그램 폴더로 끌어다 놓으시면 됩니다.

### 처음 한 번 — 인증키 넣기

설치하고 처음 여시면 **인증키를 넣는 화면**이 나옵니다.
결제하실 때 문자·이메일로 받으신 **스무 글자**를 넣어 주세요.

```
RM-XXXXX-XXXXX-XXXXX-XXXXX
```

- **통째로 붙여넣으셔도 됩니다.** 하이픈을 빼먹으셔도, 소문자로 치셔도 알아서 받습니다
- 한 번만 넣으시면 **그 컴퓨터에서 계속** 쓰십니다. 다시 묻지 않습니다
- **인터넷이 없어도 확인됩니다.** 키를 어디로 보내지 않습니다 — 이 컴퓨터 안에서 끝납니다
- 프로그램을 지웠다 다시 까셔도, 학원 자료 폴더가 남아 있으면 **그대로 열립니다**

> 키를 못 찾으시겠으면 결제하신 문자·이메일을 확인해 주세요.
> 그래도 없으시면 accelssam.com 으로 문의해 주시면 다시 보내 드립니다.

### 태블릿 · 휴대폰에서 같은 화면 열기

연주회 당일에는 **태블릿이 훨씬 편합니다.** 순서를 넘기고 출석을 확인하시는 일을
컴퓨터 앞에 앉아서 하실 수는 없으니까요.

1. 메뉴에서 **[연주회 매니저 → 태블릿에서 열기]** 를 누릅니다
2. 프로그램을 닫았다가 다시 엽니다
3. **[설정]** 화면에 QR과 주소(`http://192.168.…`)가 나옵니다
4. 태블릿 카메라로 비추시거나 주소를 그대로 치시면 같은 화면이 열립니다

> 태블릿과 컴퓨터가 **같은 와이파이**에 있어야 합니다. 인터넷으로 나가는 것이 아니라,
> 태블릿이 학원 컴퓨터를 들여다보는 것입니다 — 자료는 그대로 컴퓨터 안에 있습니다.
>
> 켜 두면 **같은 와이파이의 다른 기기**에서도 아이 명단이 보입니다.
> 학원 와이파이를 손님과 함께 쓰신다면 **행사 날에만** 켜 두세요. 기본은 꺼져 있습니다.

> **아이콘을 누르면 무대 사진이 있는 작은 창이 먼저 뜹니다.** 「프로그램을 준비하고 있습니다」
> 라고 적혀 있고, 준비가 끝나면 그 창이 사라지면서 본 창이 열립니다.
> 학원 컴퓨터에 따라 **10~25초**가 걸립니다. 그동안 아무것도 누르지 않으셔도 됩니다 —
> 두 번 누르셔도 창이 두 개 뜨지는 않습니다.

> 설치할 때 **「Windows의 PC 보호」** 파란 창이 뜨면 **추가 정보 → 실행** 을 누르세요.
> 서명서를 사지 않은 프로그램에는 늘 나오는 안내입니다.
> (파는 쪽에서 서명서를 넣으면 이 창이 사라집니다 — `docs/SIGNING.md`)

### 눌렀는지 알 수 있게

**단추를 누르면 눈에 보이게 반응합니다.** 1px 내려앉고, 아주 살짝 줄고, 둘레에 금색 테가
번집니다. 셋이 한꺼번에 일어나므로 못 보고 지나치실 일이 없습니다.

머리띠에서 **지금 보고 계신 화면에는 금색 밑줄**이 그어집니다. 「내가 지금 설정에 들어와
있나?」를 화면 내용으로 짐작하지 않으셔도 됩니다.

카드도 마찬가지입니다. 마우스를 얹으면 살짝 떠오르고, 누르면 도로 내려앉습니다.
떠오르는 것은 **누를 수 있다는 뜻**입니다.

> 화면이 움직이는 것이 불편하시면, 윈도우 「설정 → 접근성 → 시각 효과 → 애니메이션 효과」를
> 끄시면 됩니다. 프로그램이 알아서 움직임을 멈춥니다.
...
저장하면 **연주 시각이 다시 계산되고 인쇄물 75종이 함께 바뀝니다.**
사회자 멘트는 그대로 남습니다.
...
그 아래에 **작은 그림 세 장**이 나란히 있습니다. 마음에 안 드실 때 108종을 뒤지지 않으시게,
딴 길 둘을 미리 세워 둔 것입니다.
...
- 각 그림 옆에 **동그라미 칠 자리**가 있습니다.
- 그림마다 **1 · 2 · 3 번호**가 붙고, 화면의 세 장에도 같은 번호가 붙습니다. 「종이의 2번」이
  화면에서도 2번입니다.
- 그림 아래 **네모 무늬(QR)를 휴대폰 카메라로 비추면** 그 디자인이 **골라진 채로** 열립니다.
  종이에 표시하고 다시 화면에서 찾아 누르실 필요가 없습니다.
- 원장님 혼자 정하기 어려우실 때 **선생님들께 돌려 보시기**에도 좋습니다.

> 무늬는 이 프로그램이 직접 그립니다 — 인터넷에 물어보지 않으므로 연주회장에서도 뜨고,
> 학원 이름이나 행사 이름이 밖으로 나가지 않습니다.
> 휴대폰으로 여시려면 컴퓨터와 같은 와이파이여야 하고, `휴대폰으로-보기.bat` 으로 켜 두신
> 주소에서 뽑으셔야 합니다. 못 비추셔도 화면에서 같은 번호를 누르시면 됩니다.

**셋 다 마음에 안 드실 때만** 아래를 손보세요. 아래 설명은 그때 읽으시면 됩니다.

> 고를 것이 8100가지(양식 75 × 테마 108)라는 사실 자체가 걸림돌이었습니다.
> 이제는 「고르기」가 아니라 「마음에 안 들면 바꾸기」입니다.

### 고르실 때는 **그림으로** 고르십니다

**가짓수는 하나도 줄이지 않았습니다** — 테마 108종 · 양식 75종 그대로입니다.
바뀐 것은 **보이는 방식**입니다.

예전에는 이름이 세로로 늘어서 있었습니다. 「빈 홀의 저녁」 「파치먼트 세피아」 같은 이름과
색 동그라미 세 개를 보고 결과를 그리실 수 있는 분은 없습니다. 서른두 줄을 이름으로 훑고,
하나씩 눌러 오른쪽 큰 그림을 봐야 아셨습니다.

지금은 **종이 모양 그림 격자**입니다.

- 테마마다 **바탕색 · 머리띠 · 강조색 · 제목 서체**가 종이 모양에 그대로 그려집니다.
  어두운지 밝은지, 금색인지 남색인지 훑기만 해도 아십니다.
- 양식은 **지금 고르신 테마로 그린 진짜 인쇄물 축소 그림**입니다. 3단 접지가 어떻게 접히는지,
  입장권이 몇 장인지 그림으로 보입니다.
- 이름은 **두 줄까지** 다 보입니다. 잘리지 않습니다.
- **영상 템플릿 20종**도 같습니다 — 사진이 어디에 오고 이름이 어디에 뜨는지 16:9 상자에 그립니다.
- **학원 설정**의 기본 테마도 같은 그림 격자입니다. 두 곳에서 고르는 방법이 다르면 헷갈립니다.

묶음도 원장님 말로 물어봅니다 — **「우리 연주회 분위기는 어느 쪽인가요?」**
...
### 왼쪽에서 고르기 — **[직접 고르기]** 를 누르셔야 나옵니다

세 장으로 되시는 분께는 아래가 아예 안 보입니다. 예전에는 이 화면 하나에 눌러 볼 것이
**102개**, 길이가 **6,000px**(여섯 화면)이었습니다. "이대로 뽑으셔도 됩니다" 라고 적어 놓고
바로 아래 테마 서른두 개를 펼쳐 두었으니 그 말을 믿기 어려웠습니다.

지금은 **[직접 고르기]** 단추 하나로 접혀 있습니다. 바꾸실 것이 있을 때만 누르세요.
그 안에 **양식 · 테마 · 문구 · 사진**이 다 있습니다.

- **양식 75종** — 위쪽 갈래(포스터 · 프로그램 · 초대·홍보 · 행사 당일 · 진행 문서)를 먼저 누르면
  그 갈래의 양식만 보입니다.

  포스터는 서른 종이라 한 번에 다 보시기 어렵습니다. 그래서 갈래 아래에 **결**로 한 겹 더
  나누어 두었습니다 — **그림 · 사진(실사) · 선화·판화 · 우리 학원 사진 · 글자만.**
  「사진 말고 그림으로」처럼 원장님이 실제로 하시는 생각 그대로 누르시면 됩니다.
  **전체**를 누르면 서른 종이 다시 다 나옵니다.

  포스터 갈래의 **넷째 칸부터 열두 장**이 그림 포스터입니다. 사진이나 수채 그림 한 장이
  종이를 채우고 그 위에 제목이 얹힙니다.

  | 양식 | 어떤 그림인가 |
  |---|---|
  | 무대 위 피아노 | 어두운 무대에 놓인 그랜드피아노. 가장 무난합니다 |
  | 유화 무대 | 붓 자국이 남은 유화. 인쇄했을 때 가장 고급스럽습니다 |
  | 건반 | 따뜻한 빛을 받은 건반 |
  | 아이의 손 | 건반에 얹은 아이의 손. 학부모 초청 연주회에 |
  | 빛과 실루엣 | 금빛 조명 속 피아노 실루엣. 콩쿠르·시상이 있는 무대에 |
  | 빛의 들판 | 빛이 가득한 들판에서의 연주 |
  | 수채 피아노 | 흰 종이에 그린 수채. 테마 종이색이 그대로 비칩니다 |
  | 꽃과 피아노 | 벚꽃이 흩날리는 수채. 봄 연주회와 저학년 발표회에 |
  | 여름 창가 | 열린 창과 바람에 날리는 커튼. **밝아서 잉크가 적게 듭니다** |
  | 가을 낙엽 | 낙엽이 내려앉은 피아노 |
  | 크리스마스 | 전나무와 촛불. 송년 음악회에 |
  | 금빛 축하 | 금빛 색종이가 흩날리는 무대. 새해·졸업·수료식에 |

  여기에 **열한 장이 더 늘었습니다.**

  | 갈래 | 양식 | 어떤 것인가 |
  |---|---|---|
  | **선화** | 선화 · 피아노 / 건반과 음표 / 무대 아치 | **고르신 테마 색으로 그려집니다.** 테마를 바꾸면 그림 색도 함께 바뀝니다 |
  | **일러스트** | 한 줄 선화 · 고전 동판화 · 2도 인쇄 · 수묵 · 아르데코 | 그림체가 서로 확실히 다릅니다 |
  | **사진** | 무대 · 건반 · 아이의 손 | 실제로 찍은 사진처럼 보입니다 |

  > **선화 셋**은 다른 것과 다르게 움직입니다. 그림에 색이 정해져 있지 않고
  > **테마의 강조색을 그대로 입습니다.** 남색 테마를 고르시면 남색 피아노가,
  > 버건디 테마를 고르시면 버건디 피아노가 나옵니다. 종이색도 테마 것 그대로입니다.

  > 손보실 것은 없습니다. 제목이 길면 **글씨 크기가 알아서 줄어들고**, 어느 테마를
  > 고르셔도 글씨가 그림에 묻히지 않습니다. 어두운 사진에는 검은 막을, 밝은 사진에는
  > 흰 막을 얇게 깔아 글씨를 살립니다.

  **참가 상장 (금테두리)** 도 새로 들어갔습니다. 금박 테두리를 두르고 「참 가 상」을
  **진짜 금박 무늬로 칠합니다.** 시상이 있는 연주회에 쓰세요.

  종이에는 **결**이 생겼습니다. 미색 면 종이·리넨·벨벳 사진을 아주 옅게 겹친 것이라
  눈에 띄지는 않지만, 인쇄해 보시면 단색 종이와 확실히 다릅니다.
- **테마 108종** — 성격 묶음을 먼저 고르세요.

  | 묶음 | 이런 학원에 |
  |---|---|
  | 예술회관처럼 (8) | 국립극장·예술의전당 포스터의 색. 깊은 바탕에 금 한 줄 |
  ...
  108종이라 목록만으로는 못 고릅니다. **테마 찾기** 상자에 `봄` `금색` `아이` `격식` 처럼
  적으면 걸리는 것만 나옵니다.
...
**잉크도 함께 짚어 드립니다.** 원장님은 종이만 세시는데, 연주회 전날 밤에 실제로 멈추는 것은
잉크입니다. 포스터·표지·초대장처럼 **색을 꽉 채우는 인쇄물**은 글만 있는 문서보다 잉크가
몇 배로 듭니다. 30장 넘게 뽑으시면 "컬러 잉크가 얼마나 남았는지 먼저 보세요", 100장을
넘기면 "한 통으로는 모자랍니다" 라고 말씀드립니다.

> 정확한 장수는 프린터마다 다릅니다. 제조사가 말하는 "한 통에 몇 장" 은 종이의 5%만
> 칠했을 때 값이라, 종이 전체가 색인 포스터에는 그대로 쓸 수 없습니다. 그래서 어림수로만
> 말씀드립니다. 진행표·체크리스트처럼 글만 있는 인쇄물에는 잉크 이야기를 하지 않습니다.
...
| 무엇 | 어디 | 종이 미리보기 |
|---|---|---|
| 포스터 · 프로그램 · 순서지 등 75종 | 인쇄물 디자인 → **인쇄 · PDF** | 화면이 곧 종이입니다 |
...
- **당일 운영 한 벌** — 진행표 · 준비 체크리스트 · **빔프로젝터 연결 카드** · 좌석 이름표
- **안내문 한 벌** — 학부모 안내문 · 학생 준비 안내문 · 리허설 시간표
- **사회자 한 벌** — 사회자 대본 · 당일 진행표 · 시상 명단
- **접수처 한 벌** — 좌석 배치도 · 포토존 보드 (A4 가로)

> **고르신 포스터가 그대로 들어갑니다.** 「아르데코」 포스터를 골라 두고 **관객용 한 벌**을
> 누르시면, 한 벌 안의 포스터도 아르데코로 나옵니다. 한 벌을 쓰신다고 해서 고르신 것이
> 없던 일이 되지는 않습니다.

### 종이 초대장에 참석 회신 QR

**초대장 카드**에는 참석 회신 **QR**이 함께 찍힙니다. 학부모님이 **휴대폰 카메라로 비추면**
참석 회신 화면이 바로 열립니다 — 단톡방에 링크를 따로 보내지 않으셔도 됩니다.
주소도 글로 함께 적어 두어, 못 비추시는 분은 손으로 치실 수 있습니다.

> QR은 이 프로그램이 직접 그립니다. 인터넷에 물어보지 않으므로 학원 이름과 행사 이름이
> 밖으로 나가지 않습니다.
>
> **한 가지 주의.** 학부모님 휴대폰에서 열리려면 그 주소가 **밖에서 닿는 주소**여야 합니다.
> 학원 컴퓨터에서만 켜 두신 상태(`localhost`)로 뽑으시면 학부모님 휴대폰에서는 안 열립니다.
> 같은 와이파이에서 쓰실 거면 `휴대폰으로-보기.bat` 으로 켜신 뒤 그 화면에서 뽑으세요.
...
| 초대장 카드 2매 | 단톡방 말고 손으로 건네고 싶을 때. **참석 회신 QR이 들어갑니다** |
...
| 사회자 대본 | 무대 옆은 어둡습니다. 큰 글씨로 인쇄해서 손에 쥐어 주세요. 밀렸을 때 읽을 **짧은 판**이 함께 찍힙니다 |
| 빔프로젝터 연결 카드 | 스크린에 안 나올 때 볼 종이. 눌러야 할 자판과 **가방에 챙길 것**까지. 당일 가방에 한 장 |
...
**여기서 대부분 멈추십니다.** 화면은 다 만들어 놓고, 정작 연주회장에서 선을 꽂았는데
스크린에 아무것도 안 나오거나, 원장님 노트북 화면이 그대로(순서표·메신저까지) 나갑니다.

이건 우리 프로그램이 아니라 **윈도우·맥의 일**이라 화면에서 대신 해 드릴 수가 없습니다.
그래서 무대 화면 아래에 **[빔프로젝터에 꽂았는데 스크린에 안 나옵니다]** 를 두었습니다.
연주회장에서 그것을 펴 보시면 됩니다.

| | 어떻게 |
|---|---|
| **윈도우** | 자판에서 **윈도우키( ⊞ ) + P** → **[복제]**. 원장님 화면과 스크린에 같은 것이 나옵니다 |
| **윈도우 · 순서표를 감추고 싶으실 때** | **[확장]** 을 고르고 이 창을 스크린 쪽으로 끌어다 놓은 뒤 **[전체화면]** |
| **맥북** | 오른쪽 위 **제어 센터 → 화면 미러링** 에서 빔프로젝터 이름을 고르세요 |
| **그래도 안 나올 때** | 빔프로젝터 **리모컨의 입력(Input/Source)** 을 꽂으신 자리(HDMI 1·2)로 맞추세요. **이게 가장 흔합니다** |
| **노트북에 HDMI 구멍이 없을 때** | **USB-C → HDMI 젠더**가 있어야 합니다. 연주회장에 없을 수 있으니 미리 챙기세요 |

> **당일 아침 리허설 때 한 번 해 보세요.** 아이들이 오기 전에 선을 꽂고 이 화면을 띄워
> 보시는 것이 가장 좋습니다. 그때 안 되면 고칠 시간이 있습니다.

### 연주자 화면 모양 14종

모양은 **[화면 모양 바꾸기]** 를 누르시면 나옵니다. 평소에는 지금 고른 모양의 이름만
보입니다 — 모양 14 · 사진 창 8 · 배경 10 을 한꺼번에 펼치면 서른 개가 넘습니다.
...
| **리본 띠** | 위아래로 흐르는 얇은 띠 |
| **아치 무대** | 가운데를 감싸는 큰 아치 |
| **커튼 (사진)** | 진짜 무대 커튼 사진. 어두운 화면에 |
| **건반 (사진)** | 따뜻한 빛을 받은 건반 사진 |
| **조명 (사진)** | 금빛으로 번진 객석 조명. 아이 사진 뒤에 |
| **미색 종이 (사진)** | 결이 보이는 종이. 밝은 화면에 |

> 위 열 가지는 **그려 넣은 그림**이라 아무리 크게 띄워도 흐려지지 않고 **고른 테마의 색을
> 그대로** 입습니다. 아래 넷은 **사진**이라 그림으로는 안 나오는 질감이 있습니다.
> 어느 쪽이든 프로그램 안에 들어 있어 **인터넷 없이 뜨고**, 파워포인트로 받아도 함께 갑니다.

> 사진 배경을 고르셨는데 화면 밝기가 안 맞으면(어두운 사진 + 밝은 화면) **사진이 옅게
> 깔립니다.** 아이 이름이 안 보이는 일을 막기 위한 것이니, 진하게 쓰시려면 화면 밝기를
> 사진에 맞춰 바꾸세요.
...
화면 아래 **[테마 바꾸기]** 를 누르면 인쇄물에서 쓰던 **테마 108종**이 그대로 나옵니다.
...
|---|---|
| **처음부터** | 표지 · 인사말부터 앞 30초 |
| **아이 장면부터** | 첫 아이 장면부터 30초. 사진이 어떻게 잘리는지, 이름과 곡이 어디에 뜨는지는 여기서만 보입니다 |
| **앞·가운데·끝** | 앞 · 가운데 · 끝에서 한 장면씩 **이어 붙여** 30초. 「끝까지 이 느낌인가」 를 30초에 보십니다 |

앞 30초는 대개 표지와 인사말이라, 정작 보고 싶으신 아이 화면은 그 뒤에 나옵니다.
**앞·가운데·끝**으로 담으시면 장면 사이가 건너뛴 것처럼 보이는데, 그건 이어 붙인 것이라
그렇고 **진짜 영상은 이어집니다.**

#### 보시고 마음에 안 들면 — 어디를 만지나요

맛보기가 만들어지면 그 아래에 **무엇이 마음에 안 드는지 → 어디를 만지는지**가 붙습니다.

| 이런 생각이 드시면 | 여기를 만지세요 |
|---|---|
| 글씨가 작아 안 읽힙니다 | **0 · 영상 템플릿** — 글씨가 큰 템플릿으로 |
| 사진이 이상하게 잘립니다 | **아래 [만들어질 모습]** — 그 장면을 누르면 고치기가 열립니다 |
| 너무 빨리 넘어갑니다 | **4 · 모양과 길이** — 아이 한 명당 시간을 늘리세요 |
| 음악이 없어 허전합니다 | **3 · 배경음악** — 학원에서 쓰시던 파일을 끌어다 놓으세요 |

고치신 뒤에 **다시 30초만** 만들어 보시면 됩니다. 30초면 끝나니 몇 번이든 하세요.

> 아이가 몇 안 되어 영상이 원래 짧으면 이 단추가 안 뜹니다 — 그냥 다 만드시면 되기 때문입니다.

### 영상 템플릿 20종 — **[다른 모양 19가지]** 를 누르셔야 나옵니다

평소에는 **지금 고른 것 이름만** 보입니다. 스무 가지를 늘 펼쳐 두면 "이대로 만드셔도
됩니다" 라는 말이 무색해지기 때문입니다.
...
| **순서** | `← 앞으로` `뒤로 →` 로 옮깁니다. 콘티 그림 위 화살표로도, **시간 띠에서 끌어서도** 됩니다 |
| **머무는 시간** | 초 단위로. 중요한 장면은 길게 |
| **글자 자리** | 아래 · 위 · **가운데 크게** · 글자 없이. 얼굴이 가려지면 옮기세요 |
| **이 장면으로 넘어올 때** | 겹쳐 사라지기 · 밀기 · 커지며 나타나기 · 동그랗게 열리기 등 **11가지** |
| **사진 움직임** | 다가가기 · 물러나기 · 왼쪽으로 · 오른쪽으로 · 멈춤 |
| **자막이 나타날 때** | 스르르 · 아래에서 올라오기 · 톡 튀어나오기 · **한 글자씩** · 그냥 켜기 |
| **작은 그림** | 오른쪽 위에 음표 · 하트 · 별 같은 그림 하나. **움직임**(톡 나타나기 · 살랑살랑 · 반짝이기 · 가만히)도 고릅니다 |
| **동영상에서 쓸 자리** | 동영상 장면에만 나옵니다 — 아래 「동영상은 앞을 잘라 쓰십시오」 |

**[고친 것 되돌리기]** 를 누르면 처음 자동 상태로 돌아갑니다.

> **가운데 크게**는 감동 문구를 넣을 때 씁니다 — 사진 위에 어둠을 깔고 한가운데 큰 글씨로.

> **「어울리게」로 두시는 것이 가장 낫습니다.** 넘어오는 방식과 자막 등장을 그대로 두시면
> 표지·마무리는 얌전하게, 아이들 사진은 조금 움직이게 프로그램이 섞어 씁니다.
> 서른 장면을 전부 「한 글자씩」으로 맞추면 도리어 눈이 피로해집니다.

### 시간 띠 — 끌어서 옮기고, 끌어서 늘립니다

[만들어질 모습] 창 맨 위에 **시간 띠**가 접혀 있습니다. 눌러서 펴시면 가로로 긴 띠가
나오고, 장면 하나가 네모 하나입니다. **안 여셔도 됩니다** — 아래 콘티 그림만으로도
순서와 길이는 다 바꾸실 수 있습니다.

| 하고 싶으신 일 | 어떻게 |
|---|---|
| **순서 바꾸기** | 네모를 **집어서 옮기실 자리에 놓으십시오** |
| **길이 바꾸기** | 네모의 **오른쪽 끝**을 잡고 좌우로 끄십시오. 끄는 동안 몇 초인지 아래에 나옵니다 |
| **그 장면 보기** | 네모를 그냥 **누르면** 그 자리로 가고 고치는 칸이 열립니다 |
| **지금 어디를 보고 있나** | 띠 위의 **주황색 세로줄**이 미리보기를 따라 움직입니다 |

- **네모 폭이 곧 그 장면 길이입니다.** 어느 장면이 유난히 긴지 숫자가 아니라 눈으로 보입니다.
- 끌기가 어려우시면 **펴지 않으셔도 됩니다** — 아래 콘티 그림의 `←` `→`,
  고치는 칸의 `← 앞으로` `뒤로 →`, `머무는 시간` 이 그대로 있습니다.
- 서른 장면짜리에서 다섯 번째를 스무 번째로 옮기실 때, 단추로는 열다섯 번을 눌러야 합니다.
  띠에서는 한 번입니다.

### 동영상은 앞을 잘라 쓰십시오

휴대폰으로 찍은 영상은 앞이 흔들리거나 「자, 시작」 하는 소리가 들어 있습니다.
그대로 넣으면 그 부분이 영상에 그대로 남습니다.

동영상 장면을 누르시면 고치는 칸에 **「동영상에서 쓸 자리」** 가 있습니다.

1. **어디서부터** 손잡이를 오른쪽으로 미십시오. 흔들리는 앞부분이 잘려 나갑니다.
2. 옆에 **`6.0초 → 18.0초`** 처럼 실제로 쓰이는 구간이 적힙니다.
3. **얼마나 쓸지는 「머무는 시간」이 정합니다.** 시작 자리와 끝 자리를 따로 두지 않은 것은,
   둘을 다 만지시다 「끝이 시작보다 앞」인 상태가 되기 때문입니다.

| | |
|---|---|
| **긴 영상은 앞 12초만** | 20초·30초짜리를 통째로 넣으면 아이 하나가 영상의 절반을 먹습니다. 처음에는 앞 12초만 쓰고, 뒷부분을 쓰고 싶으시면 손잡이를 미십시오 |
| **짧은 영상은 통째로** | 밀 자리가 없으면 손잡이 대신 그 이유가 적힙니다. 머무는 시간을 줄이시면 밀 자리가 생깁니다 |
| **소리도 함께** | 동영상 안의 소리는 그대로 담깁니다. 잘라 낸 자리부터 들립니다 |
...
### 함께할 분들 — 연주홀 · 반주자 · 사회자 · 드레스 · 사진 · 조율사

종이는 프로그램이 만들어 드리지만, **사람은 원장님이 부르셔야** 합니다.
그 여섯 자리를 미리 만들어 두었습니다. **비어 있는 칸이 곧 아직 안 정하신 것**입니다.

접혀 있는 줄에 `함께할 분들 2 / 6 · 아직 안 정하신 것 — 반주자 · 사회자 …` 처럼
빠진 것이 그대로 적혀 있습니다. 채우실 때만 눌러 펼치시면 됩니다.

**찾아보기** — 「우리 학원 지역」을 한 번만 넣으시면(예: 일산동구) 그 뒤로는
갈래마다 **네이버 지도 · 카카오맵 · 구글**로 그 동네를 검색해 드립니다.
나가는 것은 **지역과 갈래뿐**입니다 — 아이 명단도 사진도 나가지 않습니다.

> **반주자와 사회자는 지도에 잘 안 나옵니다.** 개인이라 사업자 등록이 없기
> 때문입니다. 그래서 이 둘은 다른 길을 알려 드립니다 — 가까운 음대 학과 사무실,
> 지역 음악 선생님 모임, 반주자 구인 카페 쪽이 빠릅니다. 사회자는 학부모·고학년
> 학생·선생님이 맡는 경우가 가장 많습니다.

**사람으로 구하실 때 — 숨고** 반주자 · 사회자 · 사진 · 조율사에는 **「숨고 열기」**
단추가 함께 나옵니다. 숨고는 **첫 화면**으로 열리고, 그 아래에 **검색창에 치실 말**
(예: 「피아노 반주」)을 적어 드립니다. 검색까지 한 번에 넣어 드리지 않는 이유는,
숨고 안쪽 주소가 바뀌면 눌러도 빈 화면이 나오기 때문입니다 — 확실한 첫 화면으로
보내 드리는 편이 낫습니다.

연주홀 대관과 드레스 대여에는 이 단추가 없습니다 — 그쪽은 재능마켓에 공급이
거의 없어서, 보내 드려 봐야 헛걸음이 됩니다.

**직접 적기** — 이름·연락처·금액·메모와 「어디까지 진행됐는지」(알아보는 중 ·
예약함 · 계약금 냄 · 끝났음)를 적어 두십시오. 적어 두신 금액은 위에 합계로 나옵니다.

**전화 · 문자** — 연락처를 적어 두시면 단추가 생깁니다. 당일에 태블릿을 들고
계실 때 번호를 찾지 않으셔도 됩니다.

**내년엔 「지난번 그대로」 한 번** — 한 번 적으신 곳은 **학원 수첩**에 남습니다.
다음 연주회의 같은 자리에 지난번 이름이 단추로 뜨고, 누르시면 연락처와 금액까지
그대로 들어옵니다. 갈래마다 여섯 곳까지 기억하고, 많이 쓰신 곳이 먼저 나옵니다.

**적으신 금액은 예산으로 그대로 갑니다.** [행사 계획 → 예산]을 여시면 대관료 ·
조율 · 사진 · 반주자 · 사회자 · 드레스 줄에 **적어 두신 금액**이 들어가 있고,
그 줄에 「하모니홀 · 적어 두신 금액」처럼 어디서 온 값인지 표시됩니다.
합계와 **권장 참가비까지 그 숫자로 다시 계산**됩니다 — 같은 금액을 두 번
적으실 일이 없습니다.

> 반주자 · 사회자 · 드레스 세 줄은 **적으시기 전까지 0원**입니다. 학원마다 있고
> 없고가 갈리는 항목이라, 어림값을 넣어 두면 **없는 비용이 잡힌 예산**을 보여
> 드리게 되기 때문입니다.

수첩은 **원장님 컴퓨터 안에만** 있습니다. 저희 서버로 올라가지 않습니다.
...
**4분 넘게 밀리면** 화면이 노랗게, **10분 넘게 밀리면** 붉어집니다.

그리고 **「사회자에게」** 칸에 그대로 전하실 말이 뜹니다. 늦었다는 사실은 이미 아시니까요.

| 상태 | 사회자에게 |
|---|---|
| 예정보다 빠름 | 아이 소개를 한 줄 더 하셔도 됩니다 |
| 예정대로 (4분 안쪽) | 지금 그대로 하시면 됩니다 |
| 조금 밀림 (4~10분) | 멘트를 짧게. 박수가 끝나면 바로 다음 아이를 부르세요 |
| 많이 밀림 (10분 이상) | 멘트는 이름과 곡만. 곡 해설은 건너뛰세요 |

**사회자 대본에 「밀렸을 때」 줄이 함께 찍혀 있습니다.** 무대 옆에서 어디를 뺄지 눈으로 찾는 것은
할 수 있는 일이 아닙니다. 같은 종이에 한 줄짜리를 나란히 찍어 두었으니, 밀리면 그 줄만
읽으시면 됩니다. 여는 말과 닫는 말도 짧은 판이 함께 있습니다.

**남은 순서로 따라잡을 수 있는지까지 셈해 드립니다.** 연주회에서 줄일 수 있는 것은 사실상
사회자 멘트뿐이고, 한 순서에서 버는 시간은 10초쯤입니다. 남은 순서가 적어 못 따라잡을
때는 그렇게 말씀드리고 **마무리 인사를 짧게 하실 준비**를 하시라고 합니다 — 헛되이
급하게만 만들지 않습니다.
...
**[이름까지 말로]** 를 켜시면 「**다음, 3번 김서연**」 처럼 읽어 드립니다.

종소리는 "무슨 일이 났다" 만 알려 주므로 결국 화면을 봐야 누구인지 아십니다. 그런데
그때 원장님 손에는 아이가 있고 눈은 무대에 있습니다. **이름까지 들으시면 화면을 아예
안 보셔도 됩니다.** 브라우저가 직접 읽으므로 소리 파일도 인터넷도 필요 없습니다.
(크롬·엣지에서 됩니다. 안 되는 브라우저면 그 자리에 그렇게 적혀 있습니다.)
...
대기실에서 필요한 것은 **다음에 누구를 데려오나** 하나뿐입니다. 밀린 시간도, 사회자 멘트도,
전체 순서도 필요 없습니다. 그런데 그것들이 다 떠 있으면 정작 필요한 것이 안 보입니다.

따라보기 화면(스태프용 주소)을 여시면 **[대기실 모드로 보기]** 가 있습니다.

- **지금 데려오실 아이**가 얼굴 사진과 함께 아주 크게 나옵니다.
- **그다음 준비**도 한 줄로 함께.
- 밀린 시간 · 사회자 멘트 · 전체 순서는 **감춥니다.**
- **[이름까지 말로]** 를 켜 두시면, 다음 아이가 바뀔 때마다 **이름을 읽어 드립니다.**
  아이를 챙기면서 화면을 안 보고 계셔도 됩니다.
- **[보통 화면으로]** 를 누르면 언제든 되돌아옵니다.

> 알림음·진동·말로 읽기는 **대기실 화면에서도** 켜실 수 있습니다. 그 설정은 그 휴대폰에만
> 담기므로, 무대 옆 화면과 따로 켜고 끄실 수 있습니다.
