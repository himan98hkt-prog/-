...
export function SystemCheck({ driver, ai }: { driver: 'demo' | 'supabase'; ai: boolean }) {
  ...
  const ALWAYS = [
    ...
    '사회자 대본 곡별 생성',
    '순서표 정밀 점검 8가지',
    '인쇄물 64종 × 테마 108종 · 인쇄 · PDF 저장',
    '리허설 소집 시각 · 참가비 · 좌석 배치 계산',
    '학부모 안내 문자 · 리허설 소집 문자 만들기',
    '준비 체크리스트 · 당일 진행표',
    '무대 화면 — 연주회 PPT 자동 생성 · 파워포인트(.pptx) 파일 만들기',
    '아이 사진을 무대 화면·PPT에 넣기 (사진 축소도 이 컴퓨터에서)',
    '감동영상 만들기 — 사진 · 동영상 · 음악을 한 편으로 (브라우저 안에서)',
    '시즌 특강 커리큘럼 · 활동지',
    '이미지 보관함 · 파일 이름으로 아이 사진 한꺼번에 짝짓기',
  ]
