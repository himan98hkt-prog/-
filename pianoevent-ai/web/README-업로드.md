...
상품 편집 → **설명**란 → 오른쪽 위 **⋮ → 코드 편집기** → 안에 있던 것을
**전부 지우고** `상세페이지-붙여넣기.html` 의 내용을 붙여넣으세요.

**스크립트가 한 줄도 없습니다.** 워드프레스가 붙여넣은 `<script>` 를 지워 버리는 일이
잦아서(보안 플러그인·`unfiltered_html` 제한), 높이를 맞추는 일을 **상세페이지 쪽이
스스로** 하도록 바꿨습니다. 상세페이지와 상품 페이지가 둘 다 accelssam.com 이라
같은 출처이고, 그래서 상세페이지가 `window.frameElement` 로 자기를 담은 틀을 직접
잡을 수 있습니다.

> ⚠️ 그래서 **`download` 폴더의 `recital-manager-detail.html` 을 새것으로 바꿔 두셔야**
> 합니다. 옛 파일이 남아 있으면 600px 에서 잘린 채로 나옵니다.

폭은 `max-width` 숫자 하나만 고치시면 됩니다 —
1080px(기본) · 960px(더 좁게) · 1200px(더 넓게) · 3000px(좌우 꽉 차게).

  · 1920px → 1056px 폭, 좌우 432px씩 (가운데)      · 768px → 704px
  · 1440px → 1056px 폭, 좌우 192px씩               · 390px → 326px

`npm run verify:embed` 로 **스크립트를 한 줄도 안 넣은 상품 페이지**를 흉내 내어
네 가지 크기에서 잘리지 않는지 잽니다.

> 상세페이지를 **다른 도메인**에 올려 부르실 때만 `상세페이지-붙여넣기-스크립트판.html`
> 을 쓰세요. 그때는 브라우저가 틀 접근을 막아서 스크립트가 필요합니다.
