...
import { PAGE_PX, getTemplate, sheetCount } from '@/lib/design/templates'
...
export default async function DesignPrintPage({
  ...
  params: { id: string }
  searchParams: { template?: string; theme?: string }
}) {
  ...
  const template = getTemplate(searchParams.template ?? event.design_template)
  ...
  const sheets = sheetCount(template.id, plan.items.length)
    ...
    <div className="min-h-screen bg-muted/50 py-8 print:bg-white print:py-0">
      ...
      <div className="mx-auto mb-5 flex max-w-[860px] flex-wrap items-center justify-between gap-3 px-4 no-print">
        <p className="text-sm text-muted-foreground">
          {template.name} · {page.label} · {sheets}장 — 인쇄 대화상자에서 <strong>배율 100%</strong>, 여백 없음으로
          두면 그대로 나옵니다.
        </p>
        <PrintButton />
      </div>
