...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  return guard(async () => {
    ...
    const mine = new Set((await repo.listStudents(params.id)).map((student) => student.id))
    const rows: { id: string; duration_sec: number }[] = []
