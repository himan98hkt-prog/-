...
/**
 * 홈 화면 아이콘을 길게 눌렀을 때 나오는 바로가기 (안드로이드).
 *
 ...
 * 위젯을 나중에 붙이더라도 여기서 만든 딥링크를 그대로 쓴다.
 *
 * 라벨은 문자열 리소스로 빼서 기기 언어를 따라간다 — 한국어 앱을 영어 폰에서
 * 쓰는 사람이 적지 않다. (iOS 쪽 현지화는 app.json 의 `locales` 가 담당한다)
 */

/**
 * `rank` 가 작을수록 목록 위에 온다.
 * 정밀 분석은 프로 기능이라 두 번째다 — 무료 사용자가 먼저 누르면 페이월만 보게 된다.
 */
const SHORTCUTS = [
  { id: 'record', link: 'record', rank: 0, resource: 'shortcut_record' },
  { id: 'precise', link: 'precise', rank: 1, resource: 'shortcut_precise' },
];

const LABELS = {
  values: {
    shortcut_record: { short: '3초 녹음', long: '바로 3초 녹음하기' },
    shortcut_precise: { short: '정밀 분석', long: '3초씩 세 번 듣고 종합' },
  },
  'values-en': {
    shortcut_record: { short: 'Record 3s', long: 'Record for 3 seconds' },
    shortcut_precise: { short: 'Precise', long: 'Three takes, one reading' },
  },
  'values-ja': {
    shortcut_record: { short: '3秒録音', long: 'すぐに3秒録音する' },
    shortcut_precise: { short: '精密分析', long: '3秒×3回を聞いてまとめる' },
  },
};
...
${Object.entries(labels)
  .map(
    ([name, { short, long }]) =>
      `  <string name="${name}_short">${short}</string>\n  <string name="${name}_long">${long}</string>`,
  )
  .join('\n')}
</resources>
`;
...
/**
 * 패키지명을 매니페스트 자리표시자(`applicationId`)로 두면 안 된다.
...
${SHORTCUTS.map(
  (shortcut) => `  <shortcut
    ...
    android:shortcutRank="${shortcut.rank}"
    android:icon="@mipmap/ic_launcher"
    android:shortcutShortLabel="@string/${shortcut.resource}_short"
    android:shortcutLongLabel="@string/${shortcut.resource}_long">
    <intent
      android:action="android.intent.action.VIEW"
      android:targetPackage="${applicationId}"
      android:targetClass="${applicationId}.MainActivity"
      android:data="${scheme}://${shortcut.link}" />
  </shortcut>`,
).join('\n')}
</shortcuts>
`;
...
/**
 * 문자열 리소스는 별도 파일(`shortcuts_strings.xml`)에 쓴다.
