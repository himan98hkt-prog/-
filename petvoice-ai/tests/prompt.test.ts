...
describe('buildPrompt', () => {
  ...
  it('상황이 비면 그렇게 명시한다', () => {
    expect(buildPrompt({ ...base, mediaType: 'audio/m4a', context: '   ' })).toContain(
      '특별한 상황 설명 없음',
    );
  });
  ...
  it('출력 언어를 명시한다', () => {
    expect(buildPrompt({ ...base, locale: 'en', mediaType: 'audio/m4a', context: '' })).toContain(
      '영어(English)',
    );
    expect(buildPrompt({ ...base, locale: 'ja', mediaType: 'audio/m4a', context: '' })).toContain(
      '일본어(日本語)',
    );
  });
  ...
  it('품종 특성을 프롬프트에 주입한다', () => {
    ...
    const siamese = buildPrompt({
      ...base,
      pet: { ...pet, type: 'CAT', breed: '샴' },
      mediaType: 'audio/m4a',
      context: '',
    });
  ...
  it('모르는 품종이면 특성 문단을 넣지 않는다', () => {
    const prompt = buildPrompt({
      ...base,
      pet: { ...pet, breed: '알수없는품종', ageMonths: 36 },
      mediaType: 'audio/m4a',
      context: '',
    });
  ...
  it('노령기에는 인지기능장애 가능성을 짚어 준다', () => {
    const senior = buildPrompt({
      ...base,
      pet: { ...pet, breed: undefined, ageMonths: 130 },
      mediaType: 'audio/m4a',
      context: '',
    });
  ...
  it('어린 개체는 사회화/에너지 특성을 넣는다', () => {
    const puppy = buildPrompt({
      ...base,
      pet: { ...pet, breed: undefined, ageMonths: 4 },
      mediaType: 'audio/m4a',
      context: '',
    });
