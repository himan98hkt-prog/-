...
/**
 * 말풍선의 말투와 길이는 언어마다 다르게 지시해야 한다.
 *
 * 원래는 세 언어 모두에 "40자 이내"를 그대로 줬는데, 영어에서 40자는
 * 일곱 단어 남짓이라 문장이 잘려 나온다. 한국어·일본어 40자와 영어 40자는
 * 담기는 내용이 전혀 다르다.
 *
 * 말투도 같은 문제다. 반려동물 1인칭은 문화권마다 관습이 뚜렷해서,
 * 지시가 없으면 번역투가 나온다. 특히 일본어는 성별을 드러내는 1인칭
 * (ぼく/わたし)을 쓰면 안 된다 — 우리는 아이의 성별을 모른다.
 */
const VOICE_STYLE: Record<Locale, string> = {
  ko: '40자 이내. 어린아이처럼 솔직하고 귀여운 반말. 호칭은 붙이지 마세요.',
  en: "90자(약 15단어) 이내. 자연스러운 구어체 영어, 축약형(I'm, don't)을 쓰세요. 딱딱한 문어체 금지.",
  ja: '40자 이내. 親しみやすい話し言葉. 一人称は使わないでください(ぼく・わたし 등 성별이 드러나는 표현 금지). 어미는 「〜だよ」「〜なの」처럼 부드럽게.',
};
...
${knowledge}
[분석 원칙]
1. ${petName}의 입장에서 솔직한 1인칭 독백 메시지를 작성하세요. 이모지는 최대 1개, 보호자를 "엄마/아빠"로 부르지 말고 호칭 없이 말하세요.
   말투와 길이: ${VOICE_STYLE[locale]}
